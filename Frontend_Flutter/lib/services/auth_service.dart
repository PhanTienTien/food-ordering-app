...
class AuthService {
  ...
  Future<String?> login(LoginRequest request) {
    return _authenticateLogin('/auth/login', request.toJson());
  }
  ...
  Future<String?> _authenticateLogin(
    String path,
    Map<String, dynamic> payload,
  ) async {
    final response = await _dio.post(path, data: payload);
    ...
    final loginResponse = LoginResponse.fromJson(Map<String, dynamic>.from(data));
